...
const int idMenuManageGlobsContext       = wxNewId();
...
BEGIN_EVENT_TABLE(ProjectManagerUI, wxEvtHandler)
    ...
    EVT_MENU(idMenuManageGlobsContext,       ProjectManagerUI::OnManageGlobsContext)
...
void ProjectManagerUI::ShowMenu(wxTreeItemId id, const wxPoint& pt)
{
    ...
    if (ftd)
    {
        ...
        if (ftd->GetKind() == FileTreeData::ftdkProject)
        {
            ...
            menu.Append(idMenuManageGlobsContext,       _("Manage globs"));
            menu.Enable(idMenuManageGlobsContext, PopUpMenuOption);
...
void ProjectManagerUI::OnManageGlobsContext(cb_unused wxCommandEvent& event)
{
    wxTreeItemId sel = GetTreeSelection();
    if (!sel.IsOk())
        return;
    ...
    if (FileTreeData* ftd = (FileTreeData*)m_pTree->GetItemData(sel))
    {
        ...
        {
            ManageGlobsDlg globManager(prj, Manager::Get()->GetAppWindow());
            PlaceWindow(&globManager);
            globManager.ShowModal();
        }
    }
